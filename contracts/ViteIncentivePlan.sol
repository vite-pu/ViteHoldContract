...
contract ViteIncentivePlan {
	...
	// 当前兑换eth总量(owner每次充值eth时会重新清零)
	uint public ethPending			= 0;
	...
	function ViteIncentivePlan(address _viteTokenAddress, address _owner) {
	...
	// 置换记录放到数组中，等到owner充值eth时再给用户转eth
	function withdrawEth(uint _amountVite) {
		...
		// uint ethAmount = _amountVite.mul(10**18).div(RATE);  // 转换为wei单位
		uint ethAmount = _amountVite.div(RATE);  //本身单位就是wei
		// 记录兑换记录，存放在数组中
		WithdrawRecord memory withdrawRecord;
		withdrawRecord.addr = msg.sender;
		withdrawRecord.viteAmount = _amountVite;
		withdrawRecords.push(withdrawRecord);

		// 记录当前兑换的eth总量
		ethPending += ethAmount;
	...
	function depositEth() payable {
		require(msg.sender == owner);
		require(msg.value >= ethPending);
		require(depositStartTime != 0);

		DepositEth(depositEthId++, owner, msg.value);

		uint len = withdrawRecords.length; 
		// 遍历所有的兑换记录，给每个地址转eth
		for (uint i = 0; i<len;i++) {
			var withdrawRecord = withdrawRecords[i];
			uint viteAmount = withdrawRecord.viteAmount;
			address user = withdrawRecord.addr;
			// uint ethAmount = viteAmount.mul(10**18).div(RATE);  // 转换为wei单位
			uint ethAmount = viteAmount.div(RATE);
			
			require(ethAmount <= ethPending);

			ethPending -= ethAmount;

			withdrawRecords[i].viteAmount = 0;  // 给该用户置0，避免重入攻击
			user.transfer(ethAmount);

			require(Token(viteTokenAddress).transfer(owner, viteAmount));
			viteSent += viteAmount;  // 记录合约转出vite总量

			WithdrawalETH(withdrawEthId++, user, ethAmount);
			Withdrawal(
				withdrawId++,
				owner,
				viteAmount
		...
		// 当前兑换eth总量和兑换记录清零
		ethPending = 0;
		delete withdrawRecords;

		uint rest = this.balance;
		if (rest > 0) {  // 转出剩余的eth回owner
			owner.transfer(rest);
		}
	}
